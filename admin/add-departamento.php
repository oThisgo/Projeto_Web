...
<input style="float: left;" type='text' name='departamento' placeholder="Nome" required>
<input style="float: left; margin-left: 50px;" class="edit-btn" type='submit' name='add' value='ADICIONAR'>
</form>

<?php

if (isset($_POST['add']) && !empty($_POST['departamento'])) {
    $nome = strtoupper(filter_input(INPUT_POST, 'departamento', FILTER_SANITIZE_FULL_SPECIAL_CHARS));
    $sql = "INSERT INTO departamentos(departamento) VALUE('$nome')";
